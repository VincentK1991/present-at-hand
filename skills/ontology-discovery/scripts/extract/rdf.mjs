...
function unescapeNtLiteral(raw) {
  return raw
    .replace(/\\\\/g, "\\")
    .replace(/\\"/g, '"')
    .replace(/\\n/g, "\n")
...
async function parseOntologyVocabularyFromTurtle(ontologyContent) {
  ...
  for (const line of lines) {
    ...
    const lm = line.match(litTripleRe);
    if (!lm) continue;

    const s = lm[1];
    const p = lm[2];
    const o = unescapeNtLiteral(lm[3]);

    if (p === rdfsLabel && !labels.has(s)) labels.set(s, o);
    if (p === rdfsComment && !comments.has(s)) comments.set(s, o);
  }
...
function collapseWhitespace(s, maxLen) {
  const collapsed = String(s ?? "").replace(/\s+/g, " ").trim();
  if (!maxLen || collapsed.length <= maxLen) return collapsed;
  return collapsed.slice(0, maxLen - 1) + "…";
}

function renderVocabEntry(uri, prefixMap, labels, comments, commentMaxLen) {
  const name = shortForm(uri, prefixMap);
  const label = labels?.get(uri);
  const comment = comments?.get(uri);
  let line = `- ${name}`;
  if (label) line += ` ("${collapseWhitespace(label, 80)}")`;
  if (comment) line += `: ${collapseWhitespace(comment, commentMaxLen)}`;
  return line;
}

function buildOntologyContext(prefixMap, vocabulary) {
  const classUris = Array.from(vocabulary.classUris).sort();
  const propertyUris = Array.from(vocabulary.propertyUris).sort();
  const prefixes = Array.from(prefixMap.entries()).sort(([a], [b]) => a.localeCompare(b));
  const labels = vocabulary.labels ?? new Map();
  const comments = vocabulary.comments ?? new Map();
  ...
  out += "\nALLOWED_CLASSES (with rdfs:label and rdfs:comment where available):\n";
  for (const uri of classUris) {
    out += renderVocabEntry(uri, prefixMap, labels, comments, 600) + "\n";
  }

  out += "\nALLOWED_PROPERTIES (with rdfs:label and rdfs:comment where available):\n";
  for (const uri of propertyUris) {
    out += renderVocabEntry(uri, prefixMap, labels, comments, 300) + "\n";
  }
